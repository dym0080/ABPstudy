...
namespace MyEventCloud.Tests
{
    public abstract class MyEventCloudTestBase : AbpIntegratedTestBase
    {
        ...
        protected override void AddModules(ITypeList<AbpModule> modules)
        {
            ...
            modules.Add<MyEventCloudTestModule>();
        }
        ...
        protected void UsingDbContext(Action<MyEventCloudDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<MyEventCloudDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyEventCloudDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyEventCloudDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyEventCloudDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyEventCloudDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<MyEventCloudDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyEventCloudDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyEventCloudDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyEventCloudDbContext, Task<T>> func)
        {
            T result;
